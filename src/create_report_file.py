...
class report:
    ...
    def __init__(self):
        # podane tagi sa rozpoznawane jako zgodne z rozporzadzeniem, slownik mowi dodatkowo
        # jaki przedrostek dopisywac gdy wystapi dany tag (np dopisze OT_ do poczatekGorySkarpy)
        self.a = 1
    ...
    def copyRange(self, startCol, startRow, endCol, endRow, sheet):
        ...
        for ranges in allMergedCells:
            if (
                ranges.min_col >= startCol
                and ranges.min_row >= startRow
                and ranges.max_col <= endCol
                and ranges.max_row <= endRow
            ):
                bounds_relative = [
                    ranges.bounds[0] + 1 - startCol,
                    ranges.bounds[1] + 1 - startRow,
                    ranges.bounds[2] + 1 - startCol,
                    ranges.bounds[3] + 1 - startRow,
                ]
    ...
    def pasteRange(
        ...
        if mergedCells:
            for merged in mergedCells:
                boundary_new = (
                    get_column_letter(merged[0] + startCol - 1)
                    + str(merged[1] + startRow - 1)
                    + ':'
                    + get_column_letter(merged[2] + startCol - 1)
                    + str(merged[3] + startRow - 1)
                )
                sheetReceiving.merge_cells(boundary_new)
        countRow = 0
        for row in sheetReceiving.iter_rows(
            min_row=startRow, max_row=endRow, min_col=startCol, max_col=endCol
        ):
            countCol = 0
            for cell in row:
    ...
    def removeMerged(self, startCol, startRow, endCol, endRow, sheet):
        ...
        # druga petla jest zrobiona specjalnie i jest potrzebna poniewaz
        # gdy komorki sa rozdzielane w pierwszej petli to losowo nie dziala
        for merged in merged_list:
            sheet.unmerge_cells(
                start_row=merged[0],
                start_column=merged[1],
                end_row=merged[2],
                end_column=merged[3]
            )
    ...
    def run(self, dict_report, file_path, out_path, conv_errors_list):
        ...
        dict_copied_frame = {
            'EGiB': {
                'header': [header_egib_data, header_egib_style, header_egib_merged],
                'body': [body_egib_data, body_egib_style, body_egib_merged],
                'bottom': [bottom_egib_data, bottom_egib_style, bottom_egib_merged],
                'prez': [prez_egib_data, prez_egib_style, prez_egib_merged],
            },
            'GESUT': {
                'header': [header_gesut_data, header_gesut_style, header_gesut_merged],
                'body': [body_gesut_data, body_gesut_style, body_gesut_merged],
                'bottom': [bottom_gesut_data, bottom_gesut_style, bottom_gesut_merged],
                'prez': [prez_egib_data, prez_gesut_style, prez_gesut_merged],
            },
            'BDOT500': {
                'header': [header_bdot500_data, header_bdot500_style, header_bdot500_merged],
                'body': [body_bdot500_data, body_bdot500_style, body_bdot500_merged],
                'bottom': [bottom_bdot500_data, bottom_bdot500_style, bottom_bdot500_merged],
                'prez': [prez_bdot500_data, prez_bdot500_style, prez_bdot500_merged],
            },
            'other': {
                'header': [header_other_data, header_other_style, header_other_merged],
                'body': [body_other_data, body_other_style, body_other_merged],
        ...
        for group_name, layers_info in dict_report.items():
            if len(layers_info) > 0:
                ...
                if key_frame == "other":
                    sheet.cell(column=1, row=start_paste_row).value = str(
                        sheet.cell(column=1, row=start_paste_row).value
                ...
                for layer_name, layer_inf in layers_info.items():
                    if layer_name in prez_layers and key_frame != "other":
                        ...
                        if key_frame != "other":
                            ...
                            if (
                                sheet.cell(column=3, row=start_paste_row).value
                                != sheet.cell(column=8, row=start_paste_row).value
                            ):
        ...
        if len(conv_errors_list) > 0:
            sheet.cell(column=1, row=start_paste_row).value = (
                'W czasie importu wystąpiły błędy. Niektóre obiekty mogły nie zostać zaimportowane. Szczegóły na następnym arkuszu.'
            )
